...
class KalmanParameterModel(nn.Module):
    def __init__(self, degree: int = 3, function_type: str = 'linear'):
        super().__init__()
        self.degree = degree
        self.transition_matrix = nn.Sequential(nn.Linear(3, 32),
                                               # nn.LayerNorm(32),
                                               nn.ReLU(),
                                               nn.Linear(32, 32),
                                               # nn.LayerNorm(32),
    ...
    def forward(self, dt: float, batch_size: int = 1):
        ...
        return transition_matrix, initial_measurement_variance, measurement_variance_smoothing
...
def generate_exponential_data():
    ...
    data = torch.exp((t + torch.randn(1) * 0.01) * torch.rand(1) * 0.2) + torch.randn_like(t) * torch.randn(1) * 0.7
    return t, data


if __name__ == '__main__':
    model = KalmanParameterModel(degree=3,
    ...
    for epoch in range(10000):
        ...
        kalman_filter = DeepKalmanFilter(initial_uncertainty=1000000,
                                         # model=model,
                                         # transition_matrix=transition_matrix,
                                         initial_measurement_variance=torch.std(y_train) ** 2,
                                         # measurement_variance_smoothing=measurement_variance_smoothing,
                                         degree=3)
        transition_matrix, _, measurement_variance_smoothing = model(x_train[1] - x_train[0], 300)
        ...
        for i in range(0, 200):
            state, variance, measurement_variance = kalman_filter.update(y_train[i], transition_matrix[i],
                                                                         measurement_variance_smoothing[i])
        ...
        measurement_variances = torch.cat(
            [torch.cat(measurement_variances).reshape(-1, 1), measurement_variances[-1].repeat(100, 1)])
        ...
        #     raise
        loss = -torch.distributions.Normal(means,
                                           torch.sqrt(variances) + torch.sqrt(measurement_variances) + 1e-10).log_prob(
            target).mean() + \
               10 * torch.mean((means - target) ** 2)
        ...
        if epoch > 989:
            with torch.no_grad():
                ...
                plt.fill_between(x_target, means - 2 * (std_dev_var + std_dev_meas),
